    ...
    <div>
        <div class="row justify-content-center" :style="{ 'pointer-events': isLoading ? 'none' : 'auto' }">
            <div>
                    ...
                    <div class="card">
                        <div>
                            <b-table
                                ...
                                :apiUrl="getRout('workphotos.index',{locale : lang})"
                            ...
                            >
                                ...
                                <template #cell(category)="{ item }">
                                    <span>{{ item.category?.name || item.category_name || item.category_id }}</span>
                                </template>
                                ...
                                <template #cell(actions)="{ item }">
                                    <button class="btn btn-danger p-1 a_position" @click="destroyItem(item.id)">
                                        <i class="bi bi-trash title_hov_top" :data-title="__(('variable.remove'))">
                                            <svg v-b-tooltip.hover xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="red" class="bi bi-trash" viewBox="0 0 16 16">
                ...
                <!-- Add Modal -->
                <b-modal v-model="show" :title="__('variable.add') + ' ' + ('Work Photo')" :header-text-variant="headerTextVariant" :header-bg-variant="headerBgVariant">
                    <b-container fluid>
                        <b-row class="mb-2">
                            <b-col class="blockInput">
                                <label class="mb-1">{{ __('variable.category') || 'Category' }}</label>
                                <b-form-select v-model="form.category_id" :options="categoryOptions" :class="{'error-border': errors['category_id']}"></b-form-select>
...
export default {
    name: 'workPhotosComponent',
    components: { Swal, Multiselect, PaginationComponent, VueLoading },
    ...
    mounted(){
        PageName.$emit('updatePageName', 'WorkPhotos');
    ...
    methods: {
        ...
        fetchCategoryOptions(){
            axios.get(route('workphotos.categories', { locale: this.lang }))
                .then(res => {
                    const list = res?.data?.data || [];
                    this.categoryOptions = list.map(c => ({ value: c.id, text: c.name || ('#'+c.id) }));
                })
        ...
        getItems(ctx){
            this.isBusy = true;
            const params = { perPage: ctx.perPage, currentPage: ctx.currentPage, sortBy: this.sortBy, sortDesc: this.sortDesc };
            return axios.get(route('workphotos.index', { locale: this.lang }) + `?page=${ctx.currentPage}`, { params })
                .then(resp => {
        ...
        // add
        onLeftChange(e){ const f = e.target.files?.[0]; if (this.leftPreview) URL.revokeObjectURL(this.leftPreview); this.leftFile = f || null; this.leftPreview = f ? URL.createObjectURL(f) : ''; },
        onRightChange(e){ const f = e.target.files?.[0]; if (this.rightPreview) URL.revokeObjectURL(this.rightPreview); this.rightFile = f || null; this.rightPreview = f ? URL.createObjectURL(f) : ''; },
        ...
        submit(){
            ...
            if(!this.form.category_id){
                this.$set(this.errors, 'category_id', this.__('variable.required') || 'Required');
            }
            if(!this.leftFile && !this.rightFile){
                this.$set(this.errors, 'image_path_left', this.__('variable.required') || 'At least one image is required');
            }
            ...
            axios.post(route('workphotos.store', { locale: this.lang }), form, { headers: { 'Content-Type':'multipart/form-data' }})
                .then(res => {
        ...
        destroyItem(id){
            Swal.fire({
                ...
                if(r.isConfirmed){
                    axios.delete(route('workphotos.destroy', { locale: this.lang, workphoto: id }))
                        .then(res => {
                            if(res?.data?.isSuccess){
